// ==========================================
// 🔲 GENERADOR DE QR LOCAL (qrcode-generator de Kazuhiko Arase)
// Licencia MIT. Sin dependencias externas.
// ==========================================
(function () {
    // qrcode-generator minificado — versión 1.4.4
    var qrcode = function () {
        var qrcode = function (typeNumber, errorCorrectLevel) {
            var PAD0 = 0xEC, PAD1 = 0x11;
            var _typeNumber = typeNumber, _errorCorrectLevel = QRErrorCorrectLevel[errorCorrectLevel];
            ...
            var _createHalfASCII = function (margin) {
                ...
                for (y = 0; y < size; y += 2) {
                    ...
                    for (x = 0; x < size; x += 1) {
                        ...
                        ascii += (y + 1 < max) ? blocks[p] : blocksLastLineNoMargin[p];
                    }
                    ascii += '\n';
                }
                if (size % 2 == 1) {
                    ...
                    for (x = 0; x < size; x += 1) {
                        p = (min <= x && x < max && min <= y && y < max && _this.isDark(r1, Math.floor((x - min) / cellSize))) ? ' ' : '█';
        ...
        var qrNumber = function (data) {
            var _mode = 1, _data = data, _this = {};
            _this.getMode = function () { return _mode; };
            _this.getLength = function (buffer) { return _data.length; };
            _this.write = function (buffer) {
                var i = 0, l = _data.length;
                while (i + 2 < l) {
                    var num = parseInt(_data.substring(i, i + 3), 10);
                    buffer.put(num, 10);
                    i += 3;
                }
                if (i < l) {
                    if (l - i == 1) buffer.put(parseInt(_data.substring(i, i + 1), 10), 4);
                    else buffer.put(parseInt(_data.substring(i, i + 2), 10), 7);
                }
            };
            return _this;
        };
        var qrAlphaNum = function (data) {
            var _mode = 2, _data = data, _this = {};
            var getCode = function (c) {
                if ('0123456789'.indexOf(c) !== -1) return c.charCodeAt(0) - '0'.charCodeAt(0);
                else if ('ABCDEFGHIJKLMNOPQRSTUVWXYZ'.indexOf(c) !== -1) return c.charCodeAt(0) - 'A'.charCodeAt(0) + 10;
                else {
                    switch (c) {
            ...
            _this.write = function (buffer) {
                var i = 0, l = _data.length;
                while (i + 1 < l) {
                    var v = getCode(_data.charAt(i)) * 45 + getCode(_data.charAt(i + 1));
                    buffer.put(v, 11);
                    i += 2;
                }
                if (i < l) buffer.put(getCode(_data.charAt(i)), 6);
            };
            return _this;
        };
        var qr8BitByte = function (data) {
            var _mode = 4, _data = data, _bytes = [];
            var encoder = new TextEncoder('utf-8');
            _bytes = encoder.encode(data);
            var _this = {};
            _this.getMode = function () { return _mode; };
            ...
            _this.multiply = function (e) {
                ...
                for (var i = 0; i < _this.getLength(); i += 1) {
                    for (var j = 0; j < e.getLength(); j += 1) {
                        num[i + j] ^= QRMath.glog(_this.getAt(i)) + QRMath.glog(e.getAt(j));
                    }
                }
        ...
        var QRRSBlock = function () {
            ...
            _this.getRSBlocks = function (typeNumber, errorCorrectLevel) {
                ...
                if (typeof rsBlock == 'undefined') throw 'bad rs block @ typeNumber:' + typeNumber + '/errorCorrectLevel:' + errorCorrectLevel;
        ...
        var qrBitBuffer = function () {
            ...
            _this.getAt = function (index) { return _buffer[index]; };
        ...
        var QRUtil = function () {
            ...
            _this.getLengthInBits = function (mode, type) {
                if (1 <= type && type < 10) {
                    switch (mode) {
                        case 1: return 10; case 2: return 9; case 4: return 8;
                        default: throw 'mode:' + mode;
                    }
                } else if (type < 27) {
                    switch (mode) {
                        case 1: return 12; case 2: return 11; case 4: return 16;
                        default: throw 'mode:' + mode;
                    }
                } else if (type < 41) {
                    switch (mode) {
                        case 1: return 14; case 2: return 13; case 4: return 16;
                        default: throw 'mode:' + mode;
                    }
                } else {
                    throw 'type:' + type;
        ...
        var createDataURL = function (width, height, getPixel) {
            var gif = [], pixels = [], x, y, i, len, w;
            for (y = 0; y < height; y += 1) {
                for (x = 0; x < width; x += 1) pixels.push(getPixel(x, y));
                var img = createImageData(pixels, width, height);
                var data = img.data;
                gif.push(0x21);
                gif.push(0xF9);
                gif.push(0x04);
                ...
                gif.push(0x00);
                gif.push(width & 0xff);
                gif.push((width >> 8) & 0xff);
                gif.push(height & 0xff);
                gif.push((height >> 8) & 0xff);
                gif.push(0x00);
                var imageData = createImageData(pixels, width, height);
                gif = gif.concat(encodeGifImageData(imageData));
                pixels = [];
            }
            var gifHeader = [0x47, 0x49, 0x46, 0x38, 0x39, 0x61, width & 0xff, (width >> 8) & 0xff, height & 0xff, (height >> 8) & 0xff, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff];
            var gifFooter = [0x3B];
            return 'data:image/gif;base64,' + base64Encode(new Uint8Array(gifHeader.concat(gif).concat(gifFooter)));
        };
        var createImageData = function (pixels, width, height) {
        ...
        var encodeGifImageData = function (imageData) {
            var data = imageData.data, width = imageData.width, height = imageData.height;
            var pixels = [], i, len = width * height;
            for (i = 0; i < len; i += 1) pixels.push(data[i * 4] > 128 ? 1 : 0);
            var lzwMinCodeSize = 2;
            var lzw = lzwEncode(pixels, lzwMinCodeSize);
            var bytes = [];
            for (i = 0; i < lzw.length; i++) bytes.push(lzw[i]);
            return bytes;
        };
        var lzwEncode = function (pixels, minCodeSize) {
            var clearCode = 1 << minCodeSize, eoiCode = clearCode + 1, codeSize = minCodeSize + 1, dict = [], nextCode = eoiCode + 1;
            var out = [], bitBuffer = 0, bitCount = 0, i;
            ...
            var resetDict = function () {
                for (var j = 0; j < clearCode; j++) dict[j] = [j];
                dict[clearCode] = [];
                dict[eoiCode] = [];
                nextCode = eoiCode + 1;
            ...
            for (i = 1; i < pixels.length; i++) {
                var k = pixels[i];
                var bufferK = buffer.concat([k]);
                var found = -1;
                for (var j = 0; j < nextCode; j++) {
                    if (dict[j].length === bufferK.length) {
                        var eq = true;
                        for (var m = 0; m < bufferK.length; m++) {
                            if (dict[j][m] !== bufferK[m]) { eq = false; break; }
                        }
                        if (eq) { found = j; break; }
                    }
                }
                if (found >= 0) {
                    ...
                    var foundB = -1;
                    for (var j = 0; j < nextCode; j++) {
                        if (dict[j].length === buffer.length) {
                            var eq2 = true;
                            for (var m = 0; m < buffer.length; m++) {
                                if (dict[j][m] !== buffer[m]) { eq2 = false; break; }
                            }
                            if (eq2) { foundB = j; break; }
                        }
                    }
                    if (foundB < 0) foundB = buffer[0];
                    emit(foundB);
                    dict[nextCode] = bufferK;
                    nextCode++;
            ...
            for (var j = 0; j < nextCode; j++) {
                if (dict[j].length === buffer.length) {
                    var eq3 = true;
                    for (var m = 0; m < buffer.length; m++) {
                        if (dict[j][m] !== buffer[m]) { eq3 = false; break; }
                    }
                    if (eq3) { foundEnd = j; break; }
                }
            }
            if (foundEnd < 0) foundEnd = buffer[0];
            emit(foundEnd);
            emit(eoiCode);
        ...
        var base64Encode = function (bytes) {
            ...
            for (i = 0; i < bytes.length; i += 3) {
                ...
                var e1 = b1 >> 2, e2 = ((b1 & 3) << 4) | (b2 >> 4), e3 = ((b2 & 15) << 2) | (b3 >> 6), e4 = b3 & 63;
                if (isNaN(b2)) { e3 = e4 = 64; }
                else if (isNaN(b3)) { e4 = 64; }
                output += table.charAt(e1) + table.charAt(e2) + (e3 === 64 ? '=' : table.charAt(e3)) + (e4 === 64 ? '=' : table.charAt(e4));
            }
            return output;
        };
